    ...
    if requested in (None, "node-package") and (root / "package.json").is_file():
        ...
        return RuntimeAdapter(
            ...
            verifier_guidance=(
                ...
                + (
                    " For TypeScript utilities — including ones with local project imports "
                    "('./x' or '@/x' aliases) — use "
                    ...
                    "Destructure what you need directly from that call, for example "
                    "const {myFn} = loadStandaloneTypeScript('path/to/module.ts'); — "
                    "never write a plain import {...} from './path/to/module' for a "
                    ".ts application file. Node cannot resolve or parse TypeScript "
                    "source that way, and the import fails before your test body runs. "
                    "This does not work for files importing an external package (e.g. 'zustand', "
                    "'react') or relying on browser-only globals (Worker, RTCPeerConnection, document). "
                    "When testing code built on ReadableStream/TransformStream, pipe "
                    "with .pipeThrough()/.pipeTo() and consume the result concurrently "
                    "via `for await (const c of readable) {...}` — never call "
                    "writer.write(...) then writer.close() and only .read() afterward "
                    "with no concurrent reader; a TransformStream will not resolve "
                    "write()/close() until something is actively reading, and that "
                    "ordering deadlocks (Node exits with code 13, 'unsettled top-level "
                    "await'), which looks like a plain test failure with no assertion."
